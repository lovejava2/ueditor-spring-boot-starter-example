...
$(document).ready(function(){
    debugger;
    $("#gname").val(obj.gname);
    $("#visible").val(obj.visible);
    $("#gift_type").val(obj.gift_type);
    $("#price").val(obj.price);
    $("#sys_getgold").val(obj.sys_getgold);
    $("#user_getgold").val(obj.user_getgold);
    $("#exp").val(obj.exp);
    $("#remark").val(obj.remark);

    if(null != obj.anieffectaddress && 0 < obj.anieffectaddress.length){
        $("#anieffectaddressDiv").show();
        $("#anieffectaddressList").append(
            "<li tabindex='0' class='el-upload-list__item is-success' style='width: 25%;margin-left: 25%' id='AnieffectaddressUrl'>" +
            "<img src='"+obj.anieffectaddress+"' alt=\"\" class='el-upload-list__item-thumbnail'>" +
            "<a class=\"el-upload-list__item-name\"><i class=\"el-icon-document\"></i><input type='hidden' name='AnieffectaddressOld'  value='"+obj.Anieffectaddress+"'></a>" +
    ...
    if(null != obj.icon && 0 < obj.icon.length){
        $("#iconDiv").show();
        $("#iconList").append(
            "<li tabindex='0' class='el-upload-list__item is-success' style='width: 25%;margin-left: 25%' id='iconUrl'>" +
            "<img src='"+obj.icon+"' alt=\"\" class='el-upload-list__item-thumbnail'>" +
            "<a class=\"el-upload-list__item-name\"><i class=\"el-icon-document\"></i><input type='hidden' name='iconOld'  value='"+obj.icon+"'></a>" +
            "<label class=\"el-upload-list__item-status-label\">" +
            "<i class=\"el-icon-upload-success el-icon-check\"></i>" +
            '' +
            "<i class='el-icon-close-tip'>按 delete 键可删除</i>" +
...
function save() {
    ...
    if(null != icon && undefined != icon && 0 < icon.length){
        icon = icon;
    }else{
        icon = obj.icon;
    }
    ...
    if(null != Anieffectaddress && undefined != Anieffectaddress && 0 < Anieffectaddress.length){
        Anieffectaddress = Anieffectaddress;
    }else{
        Anieffectaddress = obj.anieffectaddress;
    }
    debugger;
